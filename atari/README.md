We built our CQL Atari code on top of code for An Optimistic Perspective on Offline Reinforcement learning present publically at: https://github.com/google-research/batch_rl. We added the CQL regularizer on top of QR-DQN implemented at: batch_rl/multi_head/quantile_agent.py. To run CQL(H) on Atari, on the 1\% data, we can use the following command:

``
python -um batch_rl.fixed_replay.train --base_dir=log_dir --replay_dir=path_to_1_percent_datasets  --gin_files='batch_rl/fixed_replay/configs/quantile_breakout.gin' --gin_bindings='FixedReplayQuantileAgent.minq_weight=1.0' --agent_name='quantile'
```

Note that, for running different games, swapping out the `gin_files` argument works, and similarly for 10\% or the first 20M samples, experiment, the coefficient alpha an be adjusted by passing in an alternate value of `FixedReplayQuantileAgent.minq_weight` in the above command. Before running this command, the 1% and 10% datasets need to be generated, which can be done as an additional pre-processing step by loading each of the 50 buffers provided officially at https://github.com/google-research/batch_rl, and then, subsampling the first 10000 samples from each of them and storing this data as a separate buffer.

The exact readme from the previous code, we built off of is shown below.
